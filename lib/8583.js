...
const customMessageTypes = require('./messageTypes');
...
const addStaticMetaData = require('./pack/addStaticMetaData');
...
class Main {
  ...
  */
    ...
    let pds = '';
    if (!this.Msg['48'] || !this.Msg['62'] || !this.Msg['123'] || !this.Msg['124'] || !this.Msg['125']) T.toErrorObject(['There are not additional fields']);
    pds += this.Msg['48'] || '';
    pds += this.Msg['62'] || '';
    pds += this.Msg['123'] || '';
    pds += this.Msg['124'] || '';
   ...
   */
  getIsoJSON(buffer, config, encoding) {
    ...
    if (Buffer.isBuffer(buffer)) {
      ...
      let iso = this.unpack_0_127(buffer, {}, _config, encoding);
      if (iso.error) {
        return iso;
      } else {
        return iso.json;
