...
function assemble0_127_Fields() {
  ...
  if (
    ...
    mtiBuffer = new Buffer.from(cptable.utils.encode(500, mtiBuffer), 'utf8');
    let buff;
        ...
        if (this_format) {
          if (this_format.LenType === 'fixed') {
            if (this_format.ContentType === 'b') {
              ...
              if (this_format.MaxLen === this.Msg[field].length) {
                let thisBuff = Buffer.from(this.Msg[field]);
                thisBuff = new Buffer.from(cptable.utils.encode(500, thisBuff.toString('utf8')), 'utf8');
                buff = Buffer.concat([buff, thisBuff]);
              } else {
                return T.toErrorObject(['invalid length of data on field ', field]);
              }
            ...
            if (thisLen === 0) {
              ...
              if (this_format.ContentType === 'b') {
                ...
                let thisBuff = Buffer.from(this.Msg[field], 'hex');
                let actualLength = thisBuff.byteLength;
                ...
                lenIndicator = new Buffer.from(lenIndicator, 'utf8');
                lenIndicator = new Buffer.from(cptable.utils.encode(500, lenIndicator.toString('utf8')), 'utf8');
          
                buff = Buffer.concat([buff, lenIndicator, thisBuff]);
              } else {
                ...
                let thisBuff = Buffer.from(lenIndicator + this.Msg[field]);

                let str = thisBuff.toString('utf8', 0, thisBuff.byteLength);
                thisBuff = new Buffer.from(cptable.utils.encode(500, str));
                buff = Buffer.concat([buff, thisBuff]);
              }
    ...
    header.writeUInt32BE(buff.length);
    return Buffer.concat([header, buff]);
  } else {
    return state;
  }
}
