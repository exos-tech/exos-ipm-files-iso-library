 ...
 */
module.exports = function (incoming, isoJSON, config, encoding) {
  if (Buffer.isBuffer(incoming)) {
    let ASCII = "ASCII";

    let mti = encoding === ASCII
      ? new Buffer.from(incoming.slice(0, 4), 'utf8')
      : new Buffer.from(cptable.utils.decode(500, incoming.slice(0, 4)), 'utf8');

    mti = mti.toString('utf8');
    isoJSON['0'] = mti;
        ...
        if (this_format) {
          if (this_format.LenType === 'fixed') {
            if (this_format.ContentType === 'b') {
              ...
              let dataOfField = encoding === ASCII
                ? new Buffer.from(thisBuff.slice(0, this_format.MaxLen), 'utf8')
                : new Buffer.from(cptable.utils.decode(500, thisBuff.slice(0, this_format.MaxLen)), 'utf8');


              isoJSON[field] = dataOfField.toString('utf8');
              thisBuff = thisBuff.slice(
                this_format.MaxLen,
                thisBuff.byteLength
              );
            ...
            if (thisLen === 0) {
              ...
              let len = encoding === ASCII
                ? new Buffer.from(thisBuff.slice(0, thisLen), 'utf8')
                : new Buffer.from(cptable.utils.decode(500, thisBuff.slice(0, thisLen)), 'utf8');
              ...
              if (this_format.ContentType === 'b') {
                ...
                let binaryData = thisBuff.slice(0, len);
                isoJSON[field] = binaryData.toString('hex');
                ...
                let dataOfField = encoding === ASCII
                ? new Buffer.from(thisBuff.slice(0, Number(len)), 'utf8')
                : new Buffer.from(cptable.utils.decode(500, thisBuff.slice(0, Number(len))), 'utf8');

                isoJSON[field] = dataOfField.toString();
